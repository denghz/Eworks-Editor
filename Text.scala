...
class Text(init: Int) extends CharSequence {
  ...
  def this(t: Text) { this(t.length max 4); insert(0, t) }
  ...
  def getString(start: Int, nchars: Int): String = {
    assert(start >= 0 && nchars >= 0 && start + nchars <= len)
    if (start + nchars < gap) new String(buffer, start, nchars)
    else if (start >= gap)
      new String(buffer, start + max - len, nchars)
    else {
      val sb = new StringBuilder(nchars)
      sb ++= new String(buffer, start, gap - start)
      sb ++= new String(buffer, gap + max - len, nchars - gap + start)
      sb.toString()
    }
  }

  /** Return the contents of the text as a String. */
  override def toString: String = getString(0, len)
  ...
  private def makeRoom(n: Int) {
    ...
    if (gap < len)
      Array.copy(buffer, max - len + gap, newbuf, newmax - len + gap, len - gap)
    buffer = newbuf
  }
}
...
object Text {
  class Immutable private[Text] (private val contents: String)
    ...
    def charAt(index: Int): Char = contents.charAt(index)
