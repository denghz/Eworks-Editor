...
class UndoableEditor extends Editor with UndoHistory {
  private var lastChange: Change = _
  private var combineInsertionFlag = true

  override def moveCommand(dir: Int): Unit = {
    super.moveCommand(dir)
    combineInsertionFlag = false
  }
  ...
  override def insertCommand(ch: Char) {
    ...
    lastChange = new AmalgInsertion(ed.point - 1, ch, combineInsertionFlag)
    combineInsertionFlag = true
  }

  /** Record of insertion that can be amalgamated with adjacent, similar changes */
  class AmalgInsertion(val pos: Int, ch: Char, val flag: Boolean)
      extends Change {
      ...
      change match {
        case other: AmalgInsertion =>
          if (text.charAt(text.length - 1) == '\n'
              || other.pos != this.pos + this.text.length || !other.flag)
            false
          else {
  ...
  override def transposeCommand(): Boolean = {
    ...
    if (super.transposeCommand()) {
      lastChange = new Change {
        def undo(): Unit = ed.transposeChars(prevPos)

        def redo(): Unit = ed.transposeChars(prevPos)
      }
      true
    } else false
  }

  override def swapChars(): Boolean = {
    if (super.swapChars()) {
      val p = ed.point
      val m = ed.mark
      lastChange = new Change {
        def undo(): Unit = ed.swapChars(p, m)
        def redo(): Unit = ed.swapChars(p, m)
      }
      true
    } else false
  }

  override def paste(): Boolean = {
    if (super.paste()) {
      val text = new Text(ed.copyBuf)
      val p = ed.point - text.length
      lastChange = new Change {
        def undo(): Unit = ed.deleteRange(p, text.length)
        def redo(): Unit = ed.insert(p, text)
      }
      true
    } else false
  }

  override def cut(): Boolean = {
    if (super.cut()) {
      val text = new Text(ed.copyBuf)
      val p = ed.point
      lastChange = new Change {
        def undo(): Unit = ed.insert(p, text)
        def redo(): Unit = ed.deleteRange(p, text.length)
      }
      true
    } else false
  }
  ...
  override def obey(cmd: Editor.Command) {
    ...
    if (lastChange != null)
      updateHistory(new EditorChange(before, lastChange, after))
    lastChange = null
  }
  ...
  class EditorChange(private val before: ed.Memento,
                     ...
                     private var after: ed.Memento)
      extends Change {

    def undo() { change.undo(); before.restore() }
    def redo() { change.redo(); after.restore() }

    override def amalgamate(other: Change): Boolean = {
