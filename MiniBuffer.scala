...
class MiniBuffer(display: Display, val prompt: String, default: String) {
  ...
  def deleteLeft() {
    if (pos > 0) { pos -= 1; text.deleteChar(pos) } else display.beep()
  }
  def moveHome() { pos = 0 }
...
object MiniBuffer {
  ...
  val keymap: Keymap[MiniBuffer => Unit] = Keymap[MiniBuffer => Unit](
  ...
  def readString(display: Display, prompt: String, default: String): String = {
  ...
  def ask(display: Display, question: String): Boolean = {
    display.beep()
    val ans = readString(display, question + " (y/n)", null)
    ans == "y"
  }
}
